...
const DOCK_ITEMS = [
  ...
  { id: "resume",       icon: Icons.FileText, label: "Resume",       href: "/project/SUBHAJIT BANERJEEE CV.0.pdf", external: true },
];
...
export default function Dock() {
  ...
  /* ── Mobile: compact scrollable bottom bar ── */
  if (isMobile) {
    return (
      <nav
        style={{
  ...
  /* ── Desktop: macOS magnifying dock at top ── */
  return (
    <motion.div
      onMouseMove={(e) => mouseX.set(e.pageX)}
      onMouseLeave={() => mouseX.set(Infinity)}
      className="fixed top-8 left-1/2 -translate-x-1/2 z-50 flex h-20 items-center gap-5 rounded-3xl border border-white/20 bg-white/5 px-6 backdrop-blur-xl shadow-2xl"
    >
      {DOCK_ITEMS.map((item) => (
  ...
  return (
    <Link href={item.href} target={item.external ? "_blank" : undefined}>
      <motion.div
        ref={ref}
        style={{ width, y }}
        className="aspect-square rounded-full bg-white/10 border border-white/20 flex items-center justify-center hover:bg-white/20 transition-colors group relative shrink-0 shadow-xl"
      >
        <item.icon className="w-1/2 h-1/2 text-gray-200 group-hover:text-white" />

        {/* Tooltip */}
        <span className="absolute -bottom-10 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap border border-white/10 z-50">
          {item.label}
        </span>
      </motion.div>
    </Link>
